...
- (BOOL)connectWithTimeout:(NSTimeInterval)timeout error:(NSError **)errPtr
    {
        ...
        dispatch_block_t block = ^{ @autoreleasepool {
            ...
            if ([self->hostName length] == 0)
            {
                ...
                self->srvResolver = [[XMPPSRVResolver alloc] initWithdDelegate:self delegateQueue:self->xmppQueue resolverQueue:NULL];
...
- (BOOL)connectTo:(XMPPJID *)jid withAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout error:(NSError **)errPtr
    {
        ...
        dispatch_block_t block = ^{ @autoreleasepool {
            ...
            // Initailize socket
            self->asyncSocket = [[GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:self->xmppQueue];
