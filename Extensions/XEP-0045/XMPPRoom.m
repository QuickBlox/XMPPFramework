...
- (void)deactivate
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
        ...
        if ((self->state & kXMPPRoomStateJoining) || (self->state & kXMPPRoomStateJoined))
		{
			[self leaveRoom];
		}
...
- (void)joinRoomUsingNickname:(NSString *)desiredNickname history:(NSXMLElement *)history password:(NSString *)passwd
{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		{
            [self->multicastDelegate xmppRoomDidNotEnter:self withError:[NSError errorWithDomain:@"com.quickblox.xmpp" code:-1 userInfo:@{NSLocalizedDescriptionKey : @"Cannot create/join room when already creating/joining/joined."}]];
			return;
		}
...
- (void)requestUsersWithAffiliation:(NSString *)affiliation{
    dispatch_block_t block = ^{
        @autoreleasepool {
            ...
            XMPPIQ *iq = [XMPPIQ iq];
            [iq addAttributeWithName:@"id" stringValue:[NSString stringWithFormat:@"%@@%@", requestRoomUsersQueryIDPrefix, self->roomJID]];
...
- (void)createOrJoinRoom
{
    dispatch_block_t block = ^{
        @autoreleasepool {
            ...
            NSString *to = [NSString stringWithFormat:@"%@/%@", self->roomJID, self->myNickname];
            
            NSXMLElement *x = [NSXMLElement elementWithName:@"x" xmlns:QBXMPPMUCNamespaceName];
            if(self->_historyAttribute != nil){
                NSXMLElement *history = [NSXMLElement elementWithName:@"history"];
                [history addAttributeWithName:[self->_historyAttribute allKeys][0]  stringValue:[[self->_historyAttribute allValues][0] description]];
                [x addChild:history];
            }
