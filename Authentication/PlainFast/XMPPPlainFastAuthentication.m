...
@implementation XMPPPlainFastAuthentication
{
...
- (BOOL)start:(NSError **)errPtr
{
    ...
    [auth addAttributeWithName:@"mechanism" stringValue:@"PLAIN_FAST"];
    
    NSString *requestedResource = [xmppStream.myJID resource];
    
    NSXMLElement *pass = [NSXMLElement elementWithName:@"password" stringValue:base64];
    [auth addChild:pass];
    NSXMLElement *res = [NSXMLElement elementWithName:@"resource" stringValue:requestedResource];
    [auth addChild:res];
//    [auth addAttributeWithName:@"resource" stringValue:@""];
...
@implementation XMPPStream (XMPPPlainFastAuthentication)

- (BOOL)supportsPlainFastAuthentication
{
    return [self supportsAuthenticationMechanism:[XMPPPlainFastAuthentication mechanismName]];
}
